...
def song(client, message):
    ...
    try:
        ...
        m.edit("❌ Hiçbir Şey Bulunamadı.\n\nBaşka bir anahtar çalışmayı deneyin veya doğru şekilde heceleyin.")
        print(str(e))
        return
    ...
    try:
        ...
        rep = "**@GoodVibeesBot tarafından yüklendi 🎵 **"
...
async def jssong(_, message):
    ...
    m = await message.reply_text("Arıyor...")
    try:
...
async def deezsong(_, message):
    global is_downloading
    if len(message.command) < 2:
        await message.reply_text("/deezer bir argüman gerektirir.")
        return
    if is_downloading:
        await message.reply_text(
            "Başka bir indirme devam ediyor, bir süre sonra tekrar deneyin."
...
async def ytmusic(client, message: Message):
    ...
    pablo = await client.send_message(
        message.chat.id, f"`Youtube Sunucularından {urlissed} Alınıyor.Lütfen Bekleyin.`"
    )
    if not urlissed:
        await pablo.edit("Geçersiz Komut Sözdizimi, Daha Fazla Bilgi İçin Lütfen Yardım Menüsünü Kontrol Edin!")
        return
    ...
    await client.send_video(
        ...
        progress_args=(
            pablo,
            c_time,
            f"`YouTube Music'ten {urlissed} Şarkı Yükleniyor!`",
            file_stark,
        ),
    )
    await pablo.delete()
